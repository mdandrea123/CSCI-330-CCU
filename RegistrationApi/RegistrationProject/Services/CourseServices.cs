...
namespace CourseRegistration.Services
{
  public class CourseServices : ICourseServices
  {
    ...
    public List<CourseOffering> getCourseOfferingsBySemesterAndDept(String semester, String department)
    {
      ...
      if (courseOfferBySemesterAndDept.Count == 0) throw new Exception("Dept is not found!");
    ...
    {
      List<Course> courses = _repo.Courses;
      List<CoreGoal> goals = _repo.Goals;
      ...
      foreach (Course c in courses)
      {
        ...
        {
          foreach (CoreGoal g in goals)
          {
            if (g.Courses.Contains(c))
            {
              coreGoals.Add(g);
            }
          }
